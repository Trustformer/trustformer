...
Section FunctionalSpecification.
    ...
    Definition fs_transitions
        ...
        | fs_act_set => 
            {[
                let $S fs_st_pin := $I fs_in_pin;
                let $S fs_st_secret := $I fs_in_secret
            ]}
        | fs_act_test => 
            {[
                if ($S fs_st_pin ==[sz] $I fs_in_pin) then 
                    let $O fs_out_secret := $S fs_st_secret;
                    let $O fs_out_status := #1
                else 
                    let $O fs_out_status := #0
            ]}
        end.
